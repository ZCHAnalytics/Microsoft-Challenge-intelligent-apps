...
export RESOURCE_GROUP_NAME=github-pipelines-$RANDOM
export AKS_NAME=contoso-video
export ACR_NAME=contosoacr$RANDOM

echo "Searching for resource group..."
az group create -n $RESOURCE_GROUP_NAME -l uksouth # add region

echo "Creating cluster..."
az aks create \
  --resource-group $RESOURCE_GROUP_NAME \
  --name $AKS_NAME \
  --location uksouth \ # add the same region as above
  --node-count 1 \
...
echo "Creating ACR..."
az acr create -n $ACR_NAME -g $RESOURCE_GROUP_NAME --sku basic --location uksouth # same region as above
...
sed -i '' 's+!IMAGE!+'"$ACR_NAME"'.azurecr.io/contoso-website+g' kubernetes/deployment.yaml # Add `.azurecr.io` after ACR_NAME variable
...
echo "-> ACR Name: $ACR_NAME.azurecr.io" Add `.azurecr.io` after ACR_NAME variable
